...
from sailpoint.v3.models.named_constructs import NamedConstructs
...
DATEFORMATOUTPUTFORMAT_ONE_OF_SCHEMAS = ["NamedConstructs", "str"]

class DateFormatOutputFormat(BaseModel):
    """
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = DateFormatOutputFormat.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in DateFormatOutputFormat with oneOf schemas: NamedConstructs, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in DateFormatOutputFormat with oneOf schemas: NamedConstructs, str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into DateFormatOutputFormat with oneOf schemas: NamedConstructs, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into DateFormatOutputFormat with oneOf schemas: NamedConstructs, str. Details: " + ", ".join(error_messages))
        else:
            return instance
