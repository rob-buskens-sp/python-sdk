...
class TransformAttributes(BaseModel):
    ...
    # data type: IdentityAttribute
    oneof_schema_13_validator: Optional[IdentityAttribute] = None
    # data type: IndexOf
    ...
    actual_instance: Optional[Union[AccountAttribute, Base64Decode, Base64Encode, Concatenation, Conditional, DateCompare, DateFormat, DateMath, DecomposeDiacriticalMarks, E164phone, FirstValid, ISO3166, IdentityAttribute, IndexOf, LeftPad, Lookup, Lower, NameNormalizer, RandomAlphaNumeric, RandomNumeric, Reference, Replace, ReplaceAll, RightPad, Rule, Split, Static, Substring, Trim, UUIDGenerator, Upper]] = None
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if not isinstance(v, Rule):
            ...
            match += 1
        # validate data type: IdentityAttribute
        if not isinstance(v, IdentityAttribute):
            error_messages.append(f"Error! Input type `{type(v)}` is not `IdentityAttribute`")
        else:
            match += 1
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in TransformAttributes with oneOf schemas: AccountAttribute, Base64Decode, Base64Encode, Concatenation, Conditional, DateCompare, DateFormat, DateMath, DecomposeDiacriticalMarks, E164phone, FirstValid, ISO3166, IdentityAttribute, IndexOf, LeftPad, Lookup, Lower, NameNormalizer, RandomAlphaNumeric, RandomNumeric, Reference, Replace, ReplaceAll, RightPad, Rule, Split, Static, Substring, Trim, UUIDGenerator, Upper. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        try:
            ...
            error_messages.append(str(e))
        # deserialize data into IdentityAttribute
        try:
            instance.actual_instance = IdentityAttribute.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
